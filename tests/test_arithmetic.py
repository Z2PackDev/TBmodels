...
def test_add(t1, t2, k, get_model, compare_isclose):
    m1 = get_model(*t1)
    m2 = get_model(*t2)
    m3 = m1 + m2
    compare_isclose(m3.hamilton(k))
    compare_isclose(m3.eigenval(k), tag='eigenval')
...
@pytest.mark.parametrize('t2', T_VALUES)
@pytest.mark.parametrize('k', KPT)
def test_sub(t1, t2, k, get_model, compare_isclose):
    m1 = get_model(*t1)
    m2 = get_model(*t2)
    m3 = m1 - m2
    compare_isclose(m3.hamilton(k))
...
@pytest.mark.parametrize('t2', T_VALUES)
@pytest.mark.parametrize('k', KPT)
def test_sub_2(t1, t2, k, get_model, compare_isclose):
    m1 = get_model(*t1)
    m2 = get_model(*t2)
    m3 = -m1 - m2
    compare_isclose(m3.hamilton(k))
...
@pytest.mark.parametrize('k', KPT)
def test_mul(t, c, k, get_model, compare_isclose):
    m1 = get_model(*t)
    m1 *= c
    compare_isclose(m1.hamilton(k))
    compare_isclose(m1.eigenval(k), tag='eigenval')
...
@pytest.mark.parametrize('k', KPT)
def test_div(t, c, k, get_model, compare_isclose):
    m1 = get_model(*t)
    m1 /= c
    compare_isclose(m1.hamilton(k))
    compare_isclose(m1.eigenval(k), tag='eigenval')
