...
@pytest.mark.parametrize("t1", [0.6 + 0.5j, 0.0 + 1j, 2])
...
def test_simple(t1, t2, onsite, cutoff, get_model, models_equal):
    ...
    def _truncate(val):
        print(val)
        print(val if abs(val) >= cutoff else 0.0)
        return val if abs(val) >= cutoff else 0.0
    ...
    reference = get_model(
        _truncate(t1),
        _truncate(t2),
        on_site=(_truncate(onsite[0]), _truncate(onsite[1])),
    )
