...
def test_inconsistent(hr_name):
    with pytest.raises(ValueError):
        model = tbmodels.Model.from_hr_file(join(SAMPLES_DIR, hr_name))


def test_emptylines():
    """test whether the input file with some random empty lines is correctly parsed"""
    model1 = tbmodels.Model.from_hr_file(join(SAMPLES_DIR, 'wannier90_hr.dat'))
    model2 = tbmodels.Model.from_hr_file(join(SAMPLES_DIR, 'wannier90_hr_v2.dat'))
    hop1 = model1.hop
    hop2 = model2.hop
...
def test_error():
    with pytest.raises(ValueError):
        tbmodels.Model.from_hr_file(join(SAMPLES_DIR, 'hr_hamilton.dat'), occ=28, pos=[[1., 1., 1.]])
