...
def get_equivalent_k(k, supercell_size):
    return itertools.product(
        *[(np.linspace(0, 1, s, endpoint=False) + ki / s) for ki, s in zip(k, supercell_size)]
    )
...
def test_supercell_simple(get_model, t_values, supercell_size):
    ...
    for k in KPT:
        ...
        ev_folded = np.sort(np.array([model.eigenval(kval) for kval in equivalent_k]).flatten())
...
def test_supercell_simple_4d(get_model, t_values, supercell_size):
    ...
    supercell_model = model.supercell(size=supercell_size)
    for k in [(-0.12341, 0.92435, 0.32, 0.1212), (0, 0, 0, 0),
