...
def models_equal():
    ...
    def inner(model1, model2, ignore_sparsity=False):
        ...
        for k in model1.hop.keys() | model2.hop.keys():
            ...
            ).all(), f"Hoppings unequal at k={k}\nmodel1:\n{np.array(model1.hop[k])}\nmodel2:\n{np.array(model2.hop[k])}"
        assert_equal(model1.pos, model2.pos)
        if not ignore_sparsity:
            assert (
                model1._sparse == model2._sparse  # pylint: disable=protected-access
            ), "Sparsity does not match"
        return True
...
def kdotp_models_equal():
    ...
    def inner(model1, model2):
        for power in (
            model1.taylor_coefficients.keys() | model1.taylor_coefficients.keys()
        ):
            assert (
