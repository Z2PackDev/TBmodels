...
if ty.TYPE_CHECKING:
    import kwant  # pylint: disable=unused-import
...
class Model(HDF5Enabled):
    ...
    def to_kwant_lattice(self):
        ...
        """
        import kwant  # pylint: disable=import-outside-toplevel,redefined-outer-name
    ...
    def add_hoppings_kwant(
        self,
        kwant_system: "kwant.Builder",
        *,
        kwant_sublattices: ty.Optional[ty.List["kwant.builder.SiteFamily"]] = None,
    ):
        """
        ...
        ----------
        kwant_system :
            The kwant Builder whose hoppings should be populated. Note that the Builder
            must already contain the sites, since only hoppings between existing sites
            will be added.
        kwant_sublattices :
            A list of kwant site families, corresponding to the sublattices of the
            model. By default, ``self.to_kwant_lattice().sublattices`` is used. When
            this parameter is explicitly specified, the site families *must* be in the
            same order as the ones generated by :meth:`.to_kwant_lattice` - note that
        ...
        for site in kwant_system.sites():
            for i, latt in enumerate(kwant_sublattices):
                if site.family == latt:
                    indices = sublattices[i].indices
                    kwant_system[site] = on_site_mat[np.ix_(indices, indices)]
                    break
        ...
        for R, mat in self.hop.items():
            ...
            for i, s1 in enumerate(sublattices):
                for j, s2 in enumerate(sublattices):
                    ...
                    # The conjugate transpose hopping is added automatically
                    # by kwant.
